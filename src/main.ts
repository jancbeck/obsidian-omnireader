...
export default class OmnireaderPlugin extends Plugin {
	...
	async onload() {
		...
		// open modal when existing highlight is clicked
		// this.registerDomEvent(document, "mouseup", async (e: MouseEvent) => {
		// 	const view = this.app.workspace.getActiveViewOfType(MarkdownView);
		// 	const editor = view?.editor;
		...
		// 		const footnote = this.matchInlineFootnote(editor);
		...
		// 				// find first matched color that matches any possible color name
		// 				higlightColor =
